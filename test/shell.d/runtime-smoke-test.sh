...
# Each engine generation registers one handler per bar, and the bar is
# instantiated once per screen. The log spans every soft reload above, so allow
# one collision per screen past the first for each loaded generation. Anything
# beyond that is two instances on the same screen in at least one generation.
...
if (( worst > allowed_collisions )); then
  ...
  fail_with_log "each widget registers its IPC handler once per screen and generation (saw $worst, allowed $allowed_collisions)"
fi
